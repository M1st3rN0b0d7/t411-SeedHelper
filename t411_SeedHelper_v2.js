...
function seedHelper_main() {
    ...
    for (var ligne = 0; ligne <= 49; ligne++) {
...
function seedHelper_toolbar(){
    ...
    var down_unit = document.getElementsByClassName("up")[0].innerText.replace(/[^a-zA-Z]/g, "");
...
function seedHelper_monitoring(){

    var monitoringActivated = 0;

    // Récupérer état checkbox tous les 250ms

    if(monitoringActivated == 1){

        setInterval(function(){

        if(typeof(document.getElementsByClassName("recommended")[0] !== "undefined")){

            alert("[SeedHelper] Torrents pertinents détectés.");
